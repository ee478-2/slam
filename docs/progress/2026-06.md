...
## 2026-06-06 (planner-safe odom during AprilTag correction)

- Fixed the local-planner jump caused by AprilTag correction (`4582ad3`):
  `localization_manager_node.py` no longer publishes the selected fused pose
  directly on `/odom` by default.
- Exact behavior now: `/robot_pose` still follows `pose_source=auto`, so fresh
  `/global_localization/robot_pose` can update the global robot estimate.
- `/odom` defaults to `odom_source=rtabmap` and republishes continuous
  `/rtabmap/odom`, preserving the local planner's no-jump odometry input.
- The tag-corrected selected pose is published separately as `/global_odom`.
- Legacy jumpy behavior is still available with `odom_source:=selected` if
  explicitly needed.
- Rationale: local planners consume `/odom` as a locally continuous state.
  Blending or switching tag-corrected global pose into `/odom` makes the ego
  pose discontinuous when a tag appears; keeping global correction separate
  prevents the planner's coordinate conversion from suddenly changing.
- Verification: Python byte-compile, `bash -n scripts/slam_aliases.sh`,
  `roslaunch --nodes slam localization_manager.launch`, `git diff --check`,
  and `catkin_make --pkg slam` passed. No live robot topic replay was run.
